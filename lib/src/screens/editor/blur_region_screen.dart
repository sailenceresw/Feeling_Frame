...
import '../../services/blur_region_service.dart';
import 'video_result_popup.dart';

/// Hide a face / plate / logo by blurring or pixelating a draggable box.
class BlurRegionScreen extends StatefulWidget {
  const BlurRegionScreen({super.key, required this.video});

  final File video;

  @override
  State<BlurRegionScreen> createState() => _BlurRegionScreenState();
}

class _BlurRegionScreenState extends State<BlurRegionScreen> {
  ...
  double _w = 0.3;
  double _h = 0.3;
  RegionMode _mode = RegionMode.blur;
  ...
  Future<void> _apply() async {
    ...
    String? out;
    try {
      out = await BlurRegionService.apply(
        widget.video.path,
        rx,
        ry,
        rw,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Blur / Pixelate')),
      body: !ready
                      ...
                      Row(
                        children: [
                          ...
                          ChoiceChip(
                            ...
                            selected: _mode == RegionMode.blur,
                            onSelected: _rendering
                                ? null
                                : (_) =>
                                    setState(() => _mode = RegionMode.blur),
                          ),
                          const SizedBox(width: 6),
                          ChoiceChip(
                            label: const Text('Pixelate'),
                            selected: _mode == RegionMode.pixelate,
                            onSelected: _rendering
                                ? null
                                : (_) => setState(
                                    () => _mode = RegionMode.pixelate),
                          ),
                        ],
                      ),
